...
// stateField: Holds our sugggestion state in the editor 
//  create : Returns the initial value when the editor loads 
// update: called on every transcation (keystorke,ect) to potentially update the value 
...
//Todo checking code in gemini

const generatePlayload = (view: EditorView, fileName: string) => {
    ...
    return ViewPlugin.fromClass(class {
        ...
        triggerSuggestion(view: EditorView) {
            ...
            debounceTimer = window.setTimeout(async () => {
                const  playload = generatePlayload(view, fileName);
                ...
                const suggestion = await fetcher(playload,currentAbortController.signal);
...
const renderPlugin = ViewPlugin.fromClass(class {
    ...
    {
        // tell codemirror to use our decorations
